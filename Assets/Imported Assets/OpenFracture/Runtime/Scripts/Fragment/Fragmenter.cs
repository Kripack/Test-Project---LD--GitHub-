...
public static class Fragmenter
{
                                ...
                                string saveFolderPath = "")
    {
        ...
        /*List<Rigidbody> copyCustom = options.customFracture;
        if (copyCustom.Count > 0)
        {
            for (int k = 0; k < copyCustom.Count; k++)
            {
                Rigidbody rigid = GameObject.Instantiate(copyCustom[k], copyCustom[k].position, copyCustom[k].rotation, parent);
                rigid.transform.localScale = copyCustom[k].transform.lossyScale;
                Vector3 dir = (rigid.transform.position - parent.position).normalized;
                                       ...
                                       ref int i)
    {
        ...
        for(int k = 0; k < meshes.Length; k++)
        {
            ...
            fragment.layer = sourceObject.layer;
