...
public class Fracture : MonoBehaviour
{
    ...
    public void TriggerEvent()
    {
        ...
        if (fractureOptions.customFracture.Count > 0)
        {
            for (int i = 0; i < fractureOptions.customFracture.Count; i++)
            {
                Rigidbody rigid = fractureOptions.customFracture[i];
                Vector3 lossyScale = rigid.transform.lossyScale;
                rigid.transform.SetParent(this.fragmentRoot.transform);
                rigid.transform.localScale = lossyScale;
                rigid.useGravity = true;
                Vector3 dir = (rigid.transform.position - this.fragmentRoot.transform.position).normalized;
                rigid.velocity = dir * 5f;
                rigid.angularVelocity = dir * 6f;
            }
        }
